...
import {
  ...
  User as UserIcon, Loader2, TrendingUp, Target, Calendar as CalendarIcon, MapPin,
} from "lucide-react";
...
function MiDia() {
  ...
  const [dialogCliente, setDialogCliente] = useState<{ id: string; nombre: string; estado: string } | null>(null);
  ...
  const load = async () => {
    ...
    // Actividades del Plan Semanal para buckets (vencidas + próximas)
    ...
    const actMapped: ActividadPlan[] = ((actRes.data as Array<{
      ...
      cliente_correo: v.clientes?.correo ?? null,
      cliente_tipo: v.clientes?.tipo ?? "empresa",
  ...
  const buckets = useMemo(() => {
    const vencidos: ActividadPlan[] = [];
    const hoy: ActividadPlan[] = [];
    const proximos: ActividadPlan[] = [];
    for (const a of actividades) {
      const d = diasHasta(a.fecha_planificada);
      if (d === null) continue;
      if (d < 0) vencidos.push(a);
      else if (d === 0) hoy.push(a);
      else if (d <= 3) proximos.push(a);
    }
    const cmp = (a: ActividadPlan, b: ActividadPlan) => {
      const c = a.fecha_planificada.localeCompare(b.fecha_planificada);
      if (c !== 0) return c;
      return (a.hora ?? "z").localeCompare(b.hora ?? "z");
    };
    return {
  ...
  return (
    <div className="space-y-5 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <Kpi icon={CalendarClock} label="Actividades pendientes" value={String(kpis.pendientes)} tone={kpis.pendientes > 0 ? "primary" : "ok"} hint="Vencidas + hoy del Plan Semanal" />
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          />
          <BucketActividades
            title="Vencidos" description="Actividades del Plan Semanal que ya pasaron y siguen pendientes."
            icon={AlertCircle} tone="warn" items={buckets.vencidos}
            onRegistrar={(a) => setDialogCliente({ id: a.cliente_id, nombre: a.cliente_nombre, estado: a.cliente_estado })}
          />
          <BucketActividades
            title="Hoy" description="Todo lo que planificaste para hoy."
            icon={CalendarClock} tone="primary" items={buckets.hoy}
            onRegistrar={(a) => setDialogCliente({ id: a.cliente_id, nombre: a.cliente_nombre, estado: a.cliente_estado })}
          />
          <BucketActividades
            title="Próximos 3 días" description="Prepárate con anticipación."
      ...
      {dialogCliente && (
        <RegistrarContactoDialog
          ...
          clienteNombre={dialogCliente.nombre}
...
function BucketActividades({ title, description, icon: Icon, tone, items, onRegistrar }: {
  title: string; description: string; icon: typeof Sunrise; tone: "ok" | "warn" | "primary";
  items: ActividadPlan[]; onRegistrar: (a: ActividadPlan) => void;
}) {
    ...
    <Card>
      ...
      <CardContent className="space-y-2">
        {items.length === 0 ? (
          ...
          <ActividadRow key={a.id} a={a} onRegistrar={onRegistrar} />
        ))}
        <div className="pt-1">
          <Link to="/rutas" className="text-[11px] text-primary hover:underline">Planificar más actividades →</Link>
        </div>
      </CardContent>
    </Card>
  );
...
function ActividadRow({ a, onRegistrar }: { a: ActividadPlan; onRegistrar: (a: ActividadPlan) => void }) {
  const Icon = a.cliente_tipo === "empresa" ? Building2 : UserIcon;
  const d = diasHasta(a.fecha_planificada);
  const cuando =
    d === null ? a.fecha_planificada
    : d < 0 ? `Vencido ${Math.abs(d)}d`
    : d === 0 ? (a.hora ? `Hoy ${a.hora.slice(0,5)}` : "Hoy")
    : `En ${d}d${a.hora ? " · " + a.hora.slice(0,5) : ""}`;
  const tone = d === null ? "text-muted-foreground" : d < 0 ? "text-red-600" : d === 0 ? "text-primary" : "text-muted-foreground";
  return (
    <div className="flex items-center gap-2 border rounded-md p-2 hover:bg-muted/30">
      <Link to="/clientes/$id" params={{ id: a.cliente_id }} className="flex items-center gap-2 flex-1 min-w-0">
        <div className="h-8 w-8 rounded bg-primary/10 grid place-items-center shrink-0">
          <Icon className="h-4 w-4 text-primary" />
        </div>
        <div className="flex-1 min-w-0">
          <div className="font-medium text-sm truncate">{a.cliente_nombre}</div>
          <div className="text-[11px] text-muted-foreground truncate flex items-center gap-1">
            <Badge variant="outline" className="text-[9px] px-1 py-0 h-4 capitalize">{a.tipo}</Badge>
            {a.motivo || "Sin motivo"}
            {a.cliente_ciudad && <><MapPin className="h-2.5 w-2.5 ml-1" />{a.cliente_ciudad}</>}
          </div>
        </div>
      </Link>
